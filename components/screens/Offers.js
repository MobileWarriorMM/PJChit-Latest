...
export default function Offers({ navigation }) {
    ...
    var { offersListData } = useSelector((state) => state.HomeScreenReducer);

    const dispatch = useDispatch()
    const [refreshing, setRefreshing] = useState(false);
    var queryIndex = 0;
    var [endReached, setEndReached] = useState(false);
    var [imgLoader, setImgLoader] = useState([]);
    ...
    const getTheImagesFromIndex = (lastIndex) => {
        ...
        try {
            ...
            axois.get(`${API_URL}PJjewels/Api/Chit/ChitMaster/Offers/ActiveList/Type/OF/LastNum/${lastIndex}`,
                { headers: { 'Content-type': 'application/json' } }).then(res => {
                    if (res?.status === 200) {
                        var templist = [];
                        templist = res?.data;
                        templist.forEach(element => {
                            offersListData.push(element)
                        });
                        offersListData = Array.from(new Set(offersListData))
        ...
        <SafeAreaView>
            <View style={styles.mainContainer}>
                ...
                <View style={styles.appBarUIStyle}>
                    ...
                    <Text style={{ fontSize: height*0.03/fScale, fontFamily: FONTS.FONT_SEMIMODAL, color: COLORS.DARK_BLUE, paddingLeft: 10 }}>OFFERS</Text>
                </View>
                        ...
                        showsVerticalScrollIndicator={false}
                            onScroll={({ nativeEvent }) => {
                                if (isCloseToBottom(nativeEvent)) {
                                    if (offersListData[offersListData.length - 1].lineNum != queryIndex) {
                                        setEndReached(endReached = true)
                                        //getTheImagesFromIndex(offersListData[offersListData.length - 1].lineNum.toString())
                                    }
                                    queryIndex = parseInt(offersListData[offersListData.length - 1].lineNum.toString());
                                    // setTimeout(()=>{
                                    //     setEndReached(endReached = false)
                                    // },3000);
                                }
                            ...
                            {
                                offersListData.length > 0 ?
                                    <View
                                        style={{ marginTop: 5, marginBottom: 5 }}
                                    >
                                        {offersListData.map((val, index) => {
                                            var isLoad = true;
                                            return (
                                                            ...
                                                            <View style={{ position: "relative" }}>
                                                                <View style={{ position: 'absolute', flexDirection: "row", zIndex: 999 }}>
                                                                    ...
                                                                    <View style={{
                                                                        ...
                                                                        <Text style={{ color: '#f1a40c', marginLeft: 20, fontSize: win.height * 0.014/fScale, fontFamily: FONTS.FONT_SEMIMODAL }} >{"\nFrom \n" + val?.fromDate?.split("T")[0] + " To \n" +
